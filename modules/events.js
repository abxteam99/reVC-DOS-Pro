var Browser = {
    ...
    requestFullscreen(lockPointer, resizeCanvas) {
        ...
        function fullscreenChange() {
            ...
            if (getFullscreenElement() === canvasContainer) {
                ...
                if (touchWrapper && touchWrapperOriginalParent) {
    ...
    getMouseWheelDelta(event) {
        var delta = 0;
        switch (event.type) {
            case "DOMMouseScroll": delta = event.detail / 3; break;
            case "mousewheel": delta = event.wheelDelta / 120; break;
            case "wheel":
                delta = event.deltaY;
        ...
        if (Browser.pointerLock) {
            ...
            if (event.type === "touchstart" || event.type === "touchend" || event.type === "touchmove") {
                ...
                if (event.touches.length > 0) {
                    ...
                    var mouseCoords = Browser.calculateMouseCoords(firstTouch.pageX, firstTouch.pageY);
                    Browser.mouseMovementX = mouseCoords.x - Browser.mouseX;
                    Browser.mouseMovementY = mouseCoords.y - Browser.mouseY;
                    Browser.mouseX = mouseCoords.x;
                    Browser.mouseY = mouseCoords.y;
                }
    ...
    updateCanvasDimensions(canvas, wNative, hNative) {
        ...
        if (window.__lowMemoryMode) {
            ...
            if (container && container.classList && container.classList.contains('fs-container')) {
